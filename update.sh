...
(cd /opt/MinerControl/worker ;git reset --hard;git remote add origin "http://$username:$secret@mon.hcsone.ir:8929/$username/workers.git";git config --global user.email "$username@mon.hcsone.ir";git config --global user.name "$username";git pull origin master)